...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        user = auth.authenticate(username=username_or_email, password=password)  # 判断用户账号密码是否正确
        if user is None:
            if User.objects.filter(email = username_or_email).exists():
                username = User.objects.get(email=username_or_email).username   #通过邮箱获取用户名
...
class ChangeNicknameForm(forms.Form):
    nikename_new = forms.CharField(label='新的昵称',
                                   max_length=20,
                                   min_length=3,
...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again = forms.CharField(label='再输入一次新密码',
                                     min_length=6,
